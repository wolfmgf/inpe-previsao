...
// Tipagem dos dados, refletindo o que a API do INPE nos fornece
...
// Dicionário de Condições do Tempo do INPE
...
const getWeatherInfo = (tempoSigla: string | undefined) => {
  ...
  const weatherInfo = { ...weatherCodeMap[tempoSigla] }; // Clona para não modificar o original
...
export default function HomePage() {
  ...
  const MapWithNoSSR = useMemo(() => dynamic(() => import('@/components/MapLeaflet'), {
    ssr: false,
    loading: () => <div className="h-full w-full bg-muted animate-pulse flex items-center justify-center"><p>Carregando mapa...</p></div>
  }), []);
              ...
              {apiResponse && (
                <div className="w-full h-full flex flex-col justify-start pt-4 space-y-8">
                  <div className="w-full grid grid-cols-1 gap-6">
                    ...
                    {/* ESTRUTURA CORRETA DO GRÁFICO DE TEMPERATURA */}
                    ...
                    {/* ESTRUTURA CORRETA DO GRÁFICO DE UV */}
                    <div className='w-full h-[220px]'>
                  ...
                  {/* Fim da seção de resultados da previsão */}
                </div>
              )}
            </CardContent>
          </Card>
        </div>
        {/* Fim da coluna de resultados */}
      </div>
